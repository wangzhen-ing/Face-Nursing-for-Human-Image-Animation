...
import math
# NOTE(wz) different face latent injection both in spatial attention and cross attention,  without alignment.
#IN additional, face latent also concat in reference feature.
        ...
        def hacked_basic_transformer_inner_forward(
            ...
            if self.only_cross_attention:
                ...
                if MODE == "read":
                    ...
                    if do_latent_attention:
                        modify_norm_hidden_states = torch.cat(
                            [modify_norm_hidden_states, self.latent_bank[0]], dim=1
                        )

                    #NOTE(WZ) self-attention
                    hidden_states_uc = (
                        self.attn1(
                    ...
                    if write_latent:
                        self.latent_bank2.append(hidden_states)
                        # print("latent_bank2的大小为:")
                        # print(len(self.latent_bank2), len(self.latent_bank2[0]))  
                    ...
                    if self.attn2 is not None:
                        ...
                        if do_latent_attention:
                            ...
                            #         norm_hidden_states,
                            #         encoder_hidden_states=self.latent_bank2[0],
                            #         attention_mask=attention_mask,
                            #     ) + hidden_states
                            # )
                            latent_cross_hidden_states = (
                                self.attn_cross_latent[0](
                                    norm_hidden_states,
                                    encoder_hidden_states=torch.cat(
                                    [norm_hidden_states, self.latent_bank2[0]], dim=1 
                                ),
                                    attention_mask=attention_mask,
                                ) + hidden_states
                            )

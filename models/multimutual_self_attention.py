        ...
        def hacked_basic_transformer_inner_forward(
            ...
            if self.only_cross_attention:
                ...
                if MODE == "read":
                    ...
                    if write_latent:
                        self.latent_bank.append(hidden_states)
                        
                    if do_latent_attention:
                        norm_hidden_states = self.norm_latent[0](hidden_states)
                        norm_latent_states = self.norm_latent[0](self.latent_bank[0])
                        hidden_states_uc = self.attn_latent[0](
                            norm_hidden_states,
                            encoder_hidden_states=torch.cat([norm_hidden_states, norm_latent_states], dim=1),
                            attention_mask=attention_mask,
                        ) + hidden_states

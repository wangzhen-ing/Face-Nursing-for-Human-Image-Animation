...
class ReferenceAttentionControl:
        ...
        def hacked_basic_transformer_inner_forward(
            ...
            if self.only_cross_attention:
                ...
                if MODE == "read":
                    ...
                    # modify_norm_hidden_states1 = torch.cat(
                    #     [norm_hidden_states] + bank_fea1, dim=1
                    # )
                    ...
                    if do_latent_attention:
                        if not ref_idx:
                            ...
                            modify_norm_hidden_states1 = torch.cat(
                                [norm_hidden_states] + [bank_fea[1]], dim=1
                            )
                        latent_hidden_states_uc = (
                            self.attn_latent[0](
                                norm_hidden_states,
                                encoder_hidden_states=modify_norm_hidden_states1,
                                attention_mask=attention_mask,
                            ) + hidden_states
                        )
    ...
    def update1(self, writer, dtype=torch.float16):
        if self.reference_attn:
            if self.fusion_blocks == "midup":
                reader_attn_modules = [
                    module
            ...
            for r, w in zip(reader_attn_modules, writer_attn_modules):
                r.latent_bank = [v.clone().to(dtype) for v in w.bank]
                w.bank.clear()

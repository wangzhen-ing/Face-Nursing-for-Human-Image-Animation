...
from dwpose.util import draw_facepose_instantid


def process_single_image(image_path, detector, face_app):
    img_name = Path(image_path).name
    root_dir = Path(image_path).parent.parent
    save_dir = root_dir.joinpath('dwpose_iid')
    out_path = save_dir.joinpath(img_name)
    # if os.path.exists(out_path):
    #     return

    dirs = ["dwpose_iid", "dwpose_meta", "face_info", "face_images_app"]
    for dir in dirs:
    ...
    print(f'save dwpose woface to {out_path}')
    dwpose_meta = {
    ...
    if len(faces) > 0:
        ...
        dwpose_wface_pil = draw_facepose_instantid(dwpose, face_info["kps"])
        dwpose_wface_pil.save(out_path)
    else:
        dwpose.save(out_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--part", type=str, default='_test')
    parser.add_argument("-j", type=int, default=4, help="Num workers")
    args = parser.parse_args()

    num_workers = args.j

    root_dir = '/cpfs01/projects-HDD/cfff-6f3a36a0cd1e_HDD/zsy_43187/champ/datasets/archived/Processed_Local_v3'
    part = args.part

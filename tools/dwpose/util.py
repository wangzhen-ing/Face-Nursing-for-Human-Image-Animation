...
def draw_facepose_instantid(image_pil, kps, color_list=[(255,0,0), (0,255,0), (0,0,255), (255,255,0), (255,0,255)]):
    stickwidth = 3
    ...
    for idx_kp, kp in enumerate(kps):
        ...
        x, y = kp
        out_img = cv2.circle(out_img.copy(), (int(x), int(y)), 3, color, -1)

    out_img = out_img.astype(np.uint8)
    img = np.array(image_pil).astype(np.uint8)
    output_img = np.where(out_img > 0, out_img, img)
    out_img_pil = Image.fromarray(output_img)
    return out_img_pil

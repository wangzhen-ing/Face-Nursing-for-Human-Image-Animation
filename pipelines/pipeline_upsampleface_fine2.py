...
from models.paramutual_self_cross_attention6 import ReferenceAttentionControl
...
class MultiGuidanceIPFine2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print("before face motion")
        print(face_guidance_fea.shape)
        #torch.Size([1, 320, 1, 32, 32])
        #NOTE(wz)face motion features operate by gaussianSmoothing 
        if smooth_attentions:
            smoothing = GaussianSmoothing(channels=320, kernel_size=3, sigma=0.5, dim=2).cuda()
            # input = F.pad(face_guidance_fea, (1, 1, 1, 1), mode='reflect')
            face_guidance_fea = face_guidance_fea.squeeze(2)
            face_guidance_fea = F.pad(face_guidance_fea, (1, 1, 1, 1), mode='reflect')

            # face_guidance_fea = rearrange(face_guidance_fea, "b c h w -> (b c) h w")
            # print(face_guidance_fea.shape)
 
            # face_guidance_fea = face_guidance_fea.unsqueeze(1)
            face_guidance_fea = smoothing(face_guidance_fea)
            # print("after smooth")
            # print(face_guidance_fea.shape)
            face_guidance_fea = face_guidance_fea.unsqueeze(2)
            # print(face_guidance_fea.shape)
            print("after face motion")
            print(face_guidance_fea.shape)    

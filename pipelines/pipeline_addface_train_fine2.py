...
#NOTE(wz)reference image as reference image all the time
@dataclass
...
class MultiGuidanceIPFine2ImagePipeline_train(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        face_guidance_encoder = getattr(self, f"guidance_encoder_DWpose")
        face_guidance_tensor = torch.from_numpy(
            np.array(face_multi_guidance_lst[0].resize((face_image_size, face_image_size)))) / 255.
        face_guidance_tensor = face_guidance_tensor.permute(2, 0, 1).unsqueeze(0).unsqueeze(2)
        face_guidance_tensor = face_guidance_tensor.to(device, guidance_encoder.dtype)
        face_guidance_fea = face_guidance_encoder(face_guidance_tensor) 
            ...
            for i, t in enumerate(timesteps):
                ...
                if i == len(timesteps) - 1 or (
                    ...
                    if callback is not None and i % callback_steps == 0:
                        ...
                        callback(step_idx, t, face_latents)

...
#NOTE(wz) pipeline for final image
...
class MultiGuidanceIPFine2ImagePipeline_train(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ref_image_latents1 = ref_image_latents * 0.18215

        clip_img1 = self.clip_image_processor.preprocess(
            images=image, return_tensors="pt"
        ).pixel_values

        clip_image_embeds1 = self.image_encoder(
            clip_img1.to("cuda", dtype=weight_dtype)
        ).image_embeds
        image_prompt_embeds1 = clip_image_embeds1.unsqueeze(1)  # (bs, 1, d)

        region_clip_img1 = self.clip_image_processor.preprocess(
            images=face_image, return_tensors="pt"
        ).pixel_values
        ...
        region_image_embeds1 = self.image_encoder_ip(
            region_clip_img1.to("cuda", dtype=weight_dtype),
            output_hidden_states=True,
        ).image_embeds 

        uncond_image_prompt_embeds1 = self.image_encoder(
            torch.zeros_like(clip_img1).to(device, dtype=self.image_encoder.dtype)
        ).image_embeds
        uncond_image_prompt_embeds1 = uncond_image_prompt_embeds1.unsqueeze(1)

        # uncond_region_image_embeds = self.image_encoder_ip(
        #     torch.zeros_like(region_clip_img).to(device, dtype=self.image_encoder_ip.dtype),
        #     output_hidden_states=True,
        # ).hidden_states[-2]
        uncond_region_image_embeds1 = self.image_encoder_ip(
            torch.zeros_like(region_clip_img1).to(device, dtype=self.image_encoder_ip.dtype),
            output_hidden_states=True,
        ).image_embeds

        if do_classifier_free_guidance:
            image_prompt_embeds1 = torch.cat(
                [uncond_image_prompt_embeds1, image_prompt_embeds1], dim=0
            )
            region_image_embeds1 = torch.cat(
                [uncond_region_image_embeds1, region_image_embeds1], dim=0
            )
        
        ip_tokens = self.image_proj_model_fine(region_image_embeds1)
        # print("------0000----00000")
        ...
        target_image = self.decode_latents(latents2)  # (b, c, 1, h, w)
        ...
        if not return_dict:
            return (target_image, face_image1)#, upsampled_image)

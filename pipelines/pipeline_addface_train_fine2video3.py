...
#NOTE(wz) pipeline for final image,生成target image as reference image
...
class MultiGuidanceIPFine2ImagePipeline_train(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for guid_idx, face_guidance_image_lst in enumerate(face_multi_guidance_lst):
            face_guidance_tensor_lst = [torch.from_numpy(np.array(face_guidance_image.resize((face_image_size, face_image_size)))) / 255. for face_guidance_image in face_guidance_image_lst]
            face_guidance_tensor = torch.stack(face_guidance_tensor_lst, dim=0).permute(3, 0, 1, 2) #(c, f, h, w)
            face_guidance_tensor = face_guidance_tensor.unsqueeze(0)
            
            face_guidance_type = face_guidance_types[guid_idx]
            guidance_encoder = getattr(self, f"guidance_encoder_{guidance_type}")
            face_guidance_tensor = face_guidance_tensor.to(device=device, dtype=guidance_encoder.dtype)
        ...
        # face_image=face_image.squeeze(2)
        # print(image.shape)
        # torch.Size([1, 3, 1024, 1024])                                                                                                                                  | 0/1 [00:00<?, ?it/s]


        image = rearrange(
            image, "b c f h w -> (b f) c h w"
        )

        ref_image_latents1 = self.vae.encode(
            image.to(
            dtype=self.vae.dtype, device=self.vae.device
        )).latent_dist.sample()  # (bs, d, 64, 64)
        ref_image_latents1 = ref_image_latents1 * 0.18215

        clip_img = self.clip_image_processor.preprocess(
        ...
        image_prompt_embeds1 = clip_image_embeds.unsqueeze(1)  # (bs, 1, d)
        face_image = rearrange(
            face_image, "b c f h w -> (b f) c h w"
        )

        region_clip_img = self.clip_image_processor.preprocess(
            images=face_image, return_tensors="pt"
        ).pixel_values

        # region_image_embeds = self.image_encoder_ip(
        #     region_clip_img.to("cuda", dtype=weight_dtype),
        #     output_hidden_states=True,
        ...
        uncond_image_prompt_embeds = self.image_encoder(
            torch.zeros_like(clip_img).to(device, dtype=self.image_encoder.dtype)
        ).image_embeds
        uncond_image_prompt_embeds = uncond_image_prompt_embeds.unsqueeze(1)

        # uncond_region_image_embeds = self.image_encoder_ip(
        #     torch.zeros_like(region_clip_img).to(device, dtype=self.image_encoder_ip.dtype),
        #     output_hidden_states=True,
        # ).hidden_states[-2]
        uncond_region_image_embeds = self.image_encoder_ip(
            torch.zeros_like(region_clip_img).to(device, dtype=self.image_encoder_ip.dtype),
        ...
        uncond_region_image_embeds = uncond_region_image_embeds.unsqueeze(1)

        if do_classifier_free_guidance:
            image_prompt_embeds1 = torch.cat(
                [uncond_image_prompt_embeds, image_prompt_embeds1], dim=0
            )
            region_image_embeds = torch.cat(
                [uncond_region_image_embeds, region_image_embeds], dim=0
            )
        ...
        # torch.Size([514, 4, 768])
        # torch.Size([2, 257, 1280])
        # torch.Size([2, 1, 768])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == 0:
                    ...
                    # reference_control_reader.update(reference_control_writer)
                    self.reference_unet(
                        ref_image_latents1.repeat(
                            (2 if do_classifier_free_guidance else 1), 1, 1, 1
                        ),
                        torch.zeros_like(t),
                        encoder_hidden_states=image_prompt_embeds1,
                        return_dict=False,
                    )
                ...
                # 3.1 expand the latents if we are doing classifier free guidance
                latent_model_input = (
                    torch.cat([latents2] * 2) if do_classifier_free_guidance else latents2
                )
                latent_model_input = self.scheduler.scale_model_input(
                    latent_model_input, t
                )
                ...
                # compute the previous noisy sample x_t -> x_t-1
                latents2 = self.scheduler.step(
                    noise_pred, t, latents2, **extra_step_kwargs, return_dict=False
                )[0]                
        ...
        target_image = self.decode_latents(latents2)  # (b, c, f, h, w)
        ...
        if not return_dict:
            return target_image#, upsampled_image)

        return MultiGuidance2ImagePipelineOutput(videos=target_image)
